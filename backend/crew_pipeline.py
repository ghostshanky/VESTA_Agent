...
from backend.models.schemas import ClassifiedFeedback, PrioritizationScore
...
def create_classifier_agent(llm) -> Agent:
    return Agent(
        ...
        backstory="You are an expert at analyzing customer feedback and identifying patterns. "
                  "You can quickly understand the sentiment (positive, neutral, negative) and "
                  "categorize feedback into themes like 'Feature Request', 'Bug Report', 'UX Issue', "
                  "'Performance', 'Pricing', 'Support', or 'Other'.",
        llm=llm,
        verbose=True,
...
def create_evaluator_agent(llm) -> Agent:
    return Agent(
        ...
        backstory="You are a product manager with deep understanding of business priorities. "
...
def create_prioritizer_agent(llm) -> Agent:
    return Agent(
        ...
        backstory="You are a strategic product leader who synthesizes feedback analysis into "
...
def classify_feedback_mock(feedback_id: int, text: str) -> Dict[str, Any]:
    ...
    themes = ["Feature Request", "Bug Report", "UX Issue", "Performance", "Pricing", "Support"]
...
{{
    ...
    "theme": "Feature Request|Bug Report|UX Issue|Performance|Pricing|Support|Other",
...
{{
    "urgency": 1-10 (how quickly this needs to be addressed),
    "impact": 1-10 (how much business impact addressing this would have),
...
def generate_priority_report(feedback_list: List[Dict[str, Any]]) -> str:
    ...
    if llm is None or MOCK_MODE:
        ...
        for i, item in enumerate(sorted_feedback, 1):
            report += f"### {i}. {item.get('theme', 'Unknown Theme')}\n\n"
            report += f"**Priority Score:** {item.get('priority_score', 0):.2f}\n\n"
            report += f"**Urgency:** {item.get('urgency', 0)}/10 | **Impact:** {item.get('impact', 0)}/10\n\n"
            report += f"**Feedback:** {item.get('summary', item.get('text', 'No summary'))}\n\n"
            report += f"**Justification:** {item.get('justification', 'No justification')}\n\n"
            report += "---\n\n"
        
        return report
    
    feedback_summary = "\n".join([
        f"{i+1}. [{item.get('theme')}] (Priority: {item.get('priority_score', 0):.2f}, "
        f"Urgency: {item.get('urgency', 0)}, Impact: {item.get('impact', 0)})\n"
        f"   Summary: {item.get('summary', item.get('text', 'No summary'))}\n"
        f"   Justification: {item.get('justification', 'No justification')}"
